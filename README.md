...
- Отправка файлов экспорта конфигураций в GIT репозиторий (только экспорт, только нешифрованные файлы)
...
> В приведенных примерах необходимо указать корректные IP адреса станции управленяи Ansible, имя пользователя и пароль
...
> Имеется обратная совместимость с SSH-подключением к устройствам на базе RouterOS 6. Для этого необходимо в group_vars указать legacy_mode: True
